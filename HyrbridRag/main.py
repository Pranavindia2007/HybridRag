...
Simple CLI chatbot for querying the SPI spec(s). Run `python ingest.py`
first to build the indices. This is intentionally minimal -- the test
scenario generation tool will build on top of SPIRagAgent + the same
...
def main():
    ...
    agent = SPIRagAgent(backend, executor)

    print("SPI Spec Chatbot -- offline, grounded in your loaded spec(s).")
