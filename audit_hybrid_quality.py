...
Usage:
    ...
    .venv/bin/python audit_hybrid_quality.py --terms SAK-TC389QP ENDINIT SCU
...
VALID_CHUNK_TYPES = {"structural", "atomic_table", "image_analysis", "hierarchical_summary"}
...
def _query_graph_by_entity_name(graph: nx.Graph, entity_name: str) -> list[dict[str, Any]]:
...
def _fresh_chunks_by_doc(skip_reparse: bool, include_image_processing: bool) -> dict[str, list[Chunk]]:
    if skip_reparse:
        return {}
    ...
    for doc_id, pdf_path in DOCUMENT_REGISTRY.items():
        if not Path(pdf_path).exists():
            fresh[doc_id] = []
            continue
        fresh[doc_id] = build_chunks(
            doc_id,
            str(pdf_path),
            backend=backend,
            process_images=include_image_processing,
        )
    return fresh
...
def audit_chunk_quality(
    ...
    issues: list[AuditIssue],
    include_image_processing: bool = False,
) -> dict[str, Any]:
    type_counts = Counter()
    doc_counts = Counter()
    missing_metadata = []
    ...
    fresh_summary = {}
    if fresh_chunks:
        comparable_vector_chunks = vector_chunks
        if not include_image_processing:
            comparable_vector_chunks = [
                chunk for chunk in vector_chunks
                if chunk["metadata"].get("chunk_type") != "image_analysis"
            ]
        vector_ids = {chunk["chunk_id"] for chunk in comparable_vector_chunks}
        for doc_id, chunks in fresh_chunks.items():
            ...
            stale_in_vector = sorted(
                chunk["chunk_id"]
                for chunk in comparable_vector_chunks
...
def audit_graph_topology(graph: nx.Graph | None, terms: list[str], issues: list[AuditIssue]) -> dict[str, Any]:
    ...
    term_checks = {}
    for term in terms:
        hits = _query_graph_by_entity_name(graph, term)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Include cached/Ollama image analysis while rebuilding chunks for comparison.",
    )
...
def main() -> int:
    ...
    fresh_chunks = _fresh_chunks_by_doc(args.skip_reparse, args.include_image_processing)
    graph = _load_graph(FUSED_GRAPH_PATH)

    chunk_summary = audit_chunk_quality(
        vector_chunks,
        vector_count,
        fresh_chunks,
        issues,
        include_image_processing=args.include_image_processing,
    )
